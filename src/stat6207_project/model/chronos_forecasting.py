...
# Ensure the data is still sorted by isbn and q_since_first
df = df.sort_values(['isbn', 'q_since_first']).reset_index(drop=True)

# Group by isbn to treat each book as an independent time series
grouped = df.groupby('isbn', sort=False)  # sort=False preserves original order

# We will create 3 folds globally (same quarter cuts for all series)
# Find the maximum length of any series
max_length = grouped.size().max()

# Define the split points (3 folds → 2 cut points)
tscv_global = TimeSeriesSplit(n_splits=3)

splits = []
fold = 1
for train_idx, val_idx in tscv_global.split(np.arange(max_length)):
    # q_since_first cutoff for this fold
    train_max_q = train_idx[-1]  # last quarter in training
    val_max_q = val_idx[-1]  # last quarter in validation

    # For each ISBN, take all observations up to the global cutoff
    train_mask = df['q_since_first'] <= train_max_q
    val_mask = (df['q_since_first'] > train_max_q) & (df['q_since_first'] <= val_max_q)
    ...
    print(f"Fold {fold}")
    print(
        f"  Training:   q_since_first <= {train_max_q}   → {len(train_df):,} observations ({train_df['isbn'].nunique()} books)")
    print(
        f"  Validation: {train_max_q} < q <= {val_max_q}   → {len(val_df):,} observations ({val_df['isbn'].nunique()} books)")
    print(f"  Validation period covers {val_df['q_since_first'].nunique()} quarters\n")

    splits.append((train_df, val_df))
    fold += 1

# Example: access the splits later
train_1, val_1 = splits[0]
train_2, val_2 = splits[1]
train_3, val_3 = splits[2]


for col in numeric_cols:
    # Forward fill within book (uses only past or present)
    df[col] = df.groupby('isbn')[col].transform(lambda x: x.ffill())
    # If no value ever observed in the entire series, use global median (acceptable)
