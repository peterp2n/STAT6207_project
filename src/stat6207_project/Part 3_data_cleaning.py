...
def clean_isbn(isbn_value) -> Optional[str]:
    """
    Clean and standardize ISBN values.
    
    Handles:
    - Float values with .0 suffix (e.g., 9781234567890.0)
    - Scientific notation (e.g., 9.780007e+12)
    - Hyphens and non-digit characters
    - None/NaN values
    """
    # Handle None/NaN
    if isbn_value is None or (isinstance(isbn_value, str) and isbn_value.strip() == ''):
        return None
    if pd.isna(isbn_value):
        return None
    
    # Convert to string, handling float/scientific notation
    if isinstance(isbn_value, (int, float)):
        # Convert scientific notation to full number
        isbn_str = f"{float(isbn_value):.0f}"
        ...
        isbn_str = remove_invisible_chars(isbn_str)
        # Remove .0 suffix if present
        if isbn_str.endswith('.0'):
            isbn_str = isbn_str[:-2]
    
    # Remove all non-digit characters (hyphens, spaces, etc.)
    cleaned = re.sub(r'\D', '', isbn_str)
    return cleaned if cleaned else None
...
def clean_amazon_csv(
    input_path: str = 'amazon_scraped_products_excel.csv',
    output_path: str = 'amazon_cleaned.csv'
) -> pd.DataFrame:
    """
    ...
    # Force ISBN columns to be read as strings to prevent float conversion
    dtype_spec = {
        'isbn_10': str,
        'isbn_13': str,
        'barcode': str
    }
    df = pd.read_csv(input_path, encoding='utf-8-sig', dtype=dtype_spec)
    print(f"✓ Loaded {len(df)} rows with {len(df.columns)} columns")

    print("\nCleaning data...")
    df_cleaned = clean_amazon_dataframe(df)

    # Ensure ISBN columns are saved as strings (not float)
    for col in ['isbn_10', 'isbn_13', 'barcode']:
        if col in df_cleaned.columns:
            # Convert to string, handle NaN -> empty string
...
def generate_data_quality_report(df: pd.DataFrame) -> None:
    """Generate a data quality report showing missing values and statistics."""
    print("\n" + "=" * 80)
    ...
    text_cols = ['product_name', 'description', 'author', 'publisher']
    ...
    if len(existing_text_cols) > 0:
        ...
        for col in existing_text_cols:
            if col in df.columns:
                ...
                print(f"{col}: {non_null} non-null ({non_null/len(df)*100:.1f}%), avg length: {avg_length:.0f} chars")
...
def main():
    ...
    try:
        ...
        df_cleaned = clean_amazon_csv(
            input_path='amazon_scraped_products_excel.csv',
            output_path='amazon_cleaned.csv'
        )

        # Generate data quality report
        generate_data_quality_report(df_cleaned)

        print("\n✅ Data cleaning completed successfully!")
        print(f"   Input:  amazon_scraped_products_excel.csv")
        print(f"   Output: amazon_cleaned.csv")
        print(f"   Rows:   {len(df_cleaned)}")
        print(f"   Columns: {len(df_cleaned.columns)}")

    except FileNotFoundError as e:
        print(f"\n❌ Error: {e}")
        print("   Make sure 'amazon_scraped_products_excel.csv' exists in the current directory.")
